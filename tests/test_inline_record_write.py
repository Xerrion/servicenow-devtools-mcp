...
async def test_write_schema_has_only_inline_input(server: MCPServer) -> None:
    schemas = {tool.name: tool for tool in await server.list_tools()}
    assert set(schemas["record_write"].input_schema["properties"]) == {"action", "table", "sys_id", "data", "preview"}
...
async def test_inline_xml_rejected_before_token_or_mutation(
    server: MCPServer, action: str, preview: bool, content: Any
) -> None:
    mock_metadata({"u_markup": "xml"})
    tools = get_tool_functions(server)
    ...
    assert all(call.request.url.path.rsplit("/", 1)[-1] in {"sys_db_object", "sys_dictionary"} for call in respx.calls)


@pytest.mark.parametrize("action", ["create", "update"])
@pytest.mark.parametrize("preview", [True, False])
@respx.mock
async def test_multiple_inline_fields_round_trip(server: MCPServer, action: str, preview: bool) -> None:
    payload = {
        "name": "Widget",
...
@respx.mock
async def test_metadata_only_update_does_not_send_script(server: MCPServer) -> None:
...
@pytest.mark.parametrize("preview", [True, False])
@respx.mock
async def test_inline_size_limit_precedes_metadata(server: MCPServer, action: str, preview: bool) -> None:
    ...
    with patch.object(PreviewTokenStore, "create", new_callable=AsyncMock) as create_token:
        result = decode_response(
            await get_tool_functions(server)["record_write"](
                action=action,
                table="sys_script",
                sys_id=SYS_ID if action == "update" else "",
                data=payload,
...
@respx.mock
async def test_inline_size_boundary_is_accepted(server: MCPServer) -> None:
...
@respx.mock
async def test_invalid_payload_precedes_metadata(server: MCPServer, payload: str) -> None:
    result = decode_response(
        await get_tool_functions(server)["record_write"](
            action="create",
            table="sys_script",
...
@respx.mock
async def test_metadata_error_blocks_write(server: MCPServer, status_code: int) -> None:
    respx.get(f"{BASE_URL}/sys_db_object").respond(200, json={"result": []})
...
@respx.mock
async def test_inherited_xml_is_validated(server: MCPServer) -> None:
...
@respx.mock
async def test_apply_rechecks_production_gate(server: MCPServer, settings: Settings) -> None:
    mock_metadata({"script": "script"})
    tools = get_tool_functions(server)
...
@respx.mock
async def test_missing_mandatory_field_blocks_create(server: MCPServer) -> None:
    respx.get(f"{BASE_URL}/sys_db_object").respond(200, json={"result": []})

    def fields(request: httpx.Request) -> httpx.Response:
        if request.url.params["sysparm_fields"] == "element,mandatory":
            assert request.url.params["sysparm_query"] == "name=sys_script^mandatory=true"
