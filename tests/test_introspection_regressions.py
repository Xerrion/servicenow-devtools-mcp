...
async def test_v1_table_lookup_joins_remote_record_trigger(settings: Settings) -> None:
    ...
    respx.get(f"{BASE_URL}/api/now/table/sys_flow_record_trigger").mock(
        return_value=httpx.Response(
            200, json={"result": [{"sys_id": remote_id, "table": "sc_task"}]}
        )
    )
...
@respx.mock
async def test_non_json_flow_response_has_safe_context(
...
async def test_filtered_triggers_use_version_specific_relations(
    ...
    respx.get(f"{BASE_URL}/api/now/table/sys_flow_record_trigger").mock(
        return_value=httpx.Response(
            200, json={"result": [{"sys_id": {"value": remote_id}}]}
        )
    )
    ...
    async with ServiceNowClient(settings, BasicAuthProvider(settings)) as client:
        assert await client.list_triggers_filtered(
            table="sc_task", trigger_type="record_update"
        ) == {"v1": [], "v2": []}
